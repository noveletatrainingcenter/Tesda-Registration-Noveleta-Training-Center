...
export default function AdminHome() {
  ...
  // Read actual CSS variable values at runtime — works in both themes
  ...
  // Dynamically size bar width based on number of data points
  const barSize = monthlyData.length <= 3 ? 48 : monthlyData.length <= 6 ? 40 : 32;
          ...
          {monthlyData.length > 0 ? (
            <ResponsiveContainer width="100%" height={220}>
              ...
              >
                <XAxis
                  dataKey="name"
                  tick={{ fontSize: 12, fill: theme === 'dark' ? '#4a5a7a' : '#94a3b8' }}
                  axisLine={false}
                  tickLine={false}
                ...
                />
                <Bar dataKey="count" fill={accentColor} radius={[6, 6, 0, 0]} maxBarSize={56} />
              </BarChart>
            </ResponsiveContainer>
          ) : (
          ...
          {data?.by_course?.slice(0, 4).map((c: any, i: number) => (
            <div key={c.course} className="flex items-center justify-between text-xs py-1.5 gap-2">
              <div className="flex items-center gap-2 min-w-0">
                ...
                <span className="text-text-secondary leading-snug break-words min-w-0">
