      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
          ...
          {monthlyData.length > 0 ? (
            ...
            <div className="h-[220px] flex items-center justify-center text-sm text-text-muted">No data available yet</div>
          )}
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
          ...
          {data?.by_course?.length > 0 ? (
            <ResponsiveContainer width="100%" height={220}>
              <PieChart>
                <Pie
                  data={data.by_course.map((e: any, i: number) => ({ ...e, fill: COLORS[i % COLORS.length] }))}
                  dataKey="count" nameKey="course" cx="50%" cy="50%" outerRadius={70}
          ...
          {data?.by_course?.slice(0, 4).map((c: any, i: number) => (
            <div key={c.course} className="flex items-center justify-between text-xs py-1">
              <div className="flex items-center gap-2">
                ...
                <span className="truncate max-w-[140px] text-text-secondary">{c.course || 'Not specified'}</span>
              </div>
