    ...
    <div className="welcome-bg min-h-screen relative overflow-hidden flex flex-col">
      ...
      <header className="relative z-10 flex items-center justify-between px-8 py-6">
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }}
          ...
          <div className="w-10 h-10 rounded-xl flex items-center justify-center bg-accent text-white text-lg font-bold">T</div>
          <div>
        ...
        >
          {features.map((f, i) => (
            <motion.div key={f.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 1 + i * 0.1 }} whileHover={{ y: -4, scale: 1.02 }}
