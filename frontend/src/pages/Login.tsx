      ...
      <div className="relative z-10 w-full max-w-md">
        ...
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div className="w-16 h-16 rounded-2xl flex items-center justify-center text-white text-3xl font-bold mx-auto mb-4"
            style={{ background: 'var(--accent)', boxShadow: '0 8px 32px color-mix(in srgb, var(--accent) 40%, transparent)' }}>T</div>
          <h1 className="font-display font-bold text-2xl" style={{ color: 'var(--text-primary)' }}>TESDA Registration</h1>
          <p className="text-sm mt-1" style={{ color: 'var(--text-muted)' }}>Noveleta Training Center</p>
        </motion.div>
          ...
          <AnimatePresence mode="wait">
            ...
            {step === 'identify' && (
              <motion.div key="identify" variants={slideVariants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.3 }}>
                <h2 className="font-display font-bold text-xl mb-1" style={{ color: 'var(--text-primary)' }}>Welcome back</h2>
                <p className="text-sm mb-6" style={{ color: 'var(--text-muted)' }}>Enter your Employee ID, username, or email to continue.</p>
                ...
                {error && <div className="flex items-center gap-2 text-sm mb-4" style={{ color: '#ef4444' }}><AlertCircle size={14} />{error}</div>}
            ...
            {step === 'login' && (
              <motion.div key="login" variants={slideVariants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.3 }}>
                <button onClick={() => { setStep('identify'); setError(''); }} className="flex items-center gap-1 text-sm mb-4" style={{ color: 'var(--text-muted)' }}>
                ...
                <div className="flex items-center gap-3 mb-6 p-3 rounded-xl" style={{ background: 'var(--bg-input)' }}>
                  <div className="w-9 h-9 rounded-lg flex items-center justify-center text-white text-sm font-bold"
                    style={{ background: role === 'admin' ? 'var(--accent)' : '#3b82f6' }}>
                ...
                <label className="label">Employee ID / Username / Email</label>
                <input className="input-base mb-4" value={identifier} readOnly style={{ opacity: 0.7 }} />
                ...
                <div className="flex items-center justify-between mb-5">
                  <label className="flex items-center gap-2 cursor-pointer text-sm" style={{ color: 'var(--text-secondary)' }}>
                    <input type="checkbox" className="w-4 h-4" style={{ accentColor: 'var(--accent)' }}
                      checked={rememberMe} onChange={e => setRememberMe(e.target.checked)} />
                    Remember me
                  </label>
                  <button type="button" className="text-sm font-medium" style={{ color: 'var(--accent)' }}
            ...
            {step === 'forgot-admin' && (
              <motion.div key="forgot-admin" variants={slideVariants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.3 }}>
                <button onClick={() => { setStep('login'); setError(''); setFpStep('email'); }} className="flex items-center gap-1 text-sm mb-4" style={{ color: 'var(--text-muted)' }}>
                  <ArrowLeft size={13} /> Back
                </button>
                <h2 className="font-display font-bold text-xl mb-1" style={{ color: 'var(--text-primary)' }}>Reset Password</h2>
                <p className="text-sm mb-6" style={{ color: 'var(--text-muted)' }}>Answer your security question to reset your password.</p>

                {fpStep === 'done' ? (
                  <div className="text-center py-4">
                    <CheckCircle size={40} className="mx-auto mb-3" style={{ color: 'var(--accent)' }} />
                    <p className="font-semibold" style={{ color: 'var(--text-primary)' }}>Password Reset!</p>
                    <p className="text-sm mt-1 mb-4" style={{ color: 'var(--text-muted)' }}>You can now log in with your new password.</p>
                    <button className="btn-primary w-full justify-center" onClick={() => { setStep('login'); setFpStep('email'); }}>Back to Login</button>
                  </div>
                ) : fpStep === 'email' ? (
                  <>
                    <label className="label">Admin Email</label>
                    <input className="input-base mb-4" type="email" placeholder="admin@tesda.gov.ph" value={fpEmail} onChange={e => setFpEmail(e.target.value)} autoFocus />
                    {error && <div className="flex items-center gap-2 text-sm mb-4" style={{ color: '#ef4444' }}><AlertCircle size={14} />{error}</div>}
                  ...
                  <>
                    <div className="p-3 rounded-xl mb-4" style={{ background: 'var(--bg-input)', border: '1px solid var(--border)' }}>
                      <p className="text-xs mb-1" style={{ color: 'var(--text-muted)' }}>Security Question:</p>
                      <p className="font-medium text-sm" style={{ color: 'var(--text-primary)' }}>{fpQuestion}</p>
                    </div>
                    <label className="label">Your Answer</label>
                    <input className="input-base mb-4" placeholder="Answer (case-insensitive)" value={fpAnswer} onChange={e => setFpAnswer(e.target.value)} />
                    <label className="label">New Password</label>
                    <input className="input-base mb-4" type="password" placeholder="New password" value={fpNewPass} onChange={e => setFpNewPass(e.target.value)} />
                    {error && <div className="flex items-center gap-2 text-sm mb-4" style={{ color: '#ef4444' }}><AlertCircle size={14} />{error}</div>}
                    <button className="btn-primary w-full justify-center" onClick={handleForgotAdmin} disabled={loading}>
            ...
            {step === 'forgot-encoder' && (
              <motion.div key="forgot-encoder" variants={slideVariants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.3 }}>
                <button onClick={() => { setStep('login'); setError(''); }} className="flex items-center gap-1 text-sm mb-4" style={{ color: 'var(--text-muted)' }}>
                  <ArrowLeft size={13} /> Back
                </button>
                <h2 className="font-display font-bold text-xl mb-1" style={{ color: 'var(--text-primary)' }}>Use Reset Ticket</h2>
                <p className="text-sm mb-6" style={{ color: 'var(--text-muted)' }}>Enter the 8-character ticket provided by your Admin.</p>
                <label className="label">Reset Ticket (8 characters)</label>
                <input className="input-base mb-4 font-mono tracking-widest text-center text-lg uppercase"
                  placeholder="XXXX-XXXX" maxLength={8} value={ticket}
                  onChange={e => setTicket(e.target.value.toUpperCase())} autoFocus />
                <label className="label">New Password</label>
                <input className="input-base mb-4" type="password" placeholder="New password" value={rtNewPass} onChange={e => setRtNewPass(e.target.value)} />
                <label className="label">Confirm Password</label>
                <input className="input-base mb-4" type="password" placeholder="Confirm password" value={rtConfirm} onChange={e => setRtConfirm(e.target.value)} />
                {error && <div className="flex items-center gap-2 text-sm mb-4" style={{ color: '#ef4444' }}><AlertCircle size={14} />{error}</div>}
